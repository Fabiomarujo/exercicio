from django import forms

# Class formulário para iniciado

class TituloForm(forms.Form):
    descricao = forms.CharField(max_length=100, required=True, help_text="Informar a descrição.")

class TituloAtualizarForm(forms.Form):
    codigo = forms.IntegerField(required=True, help_text='Informe o código do Título')
    descricao = forms.CharField(max_length=70, help_text='Informe a descrição do Titulo')